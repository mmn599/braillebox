...
// U0RX (VCP transmit) connected to PA1
#include <stdint.h>
...
void UART_Init(void){
  ...
  UART1_IBRD_R = 520;                    // IBRD = int(80,000,000 / (16 * 9600)) = int(520.83333)
  UART1_FBRD_R = 53;                     // FBRD = int(0.1267 * 64 + 0.5) = 8
  ...
  GPIO_PORTB_AMSEL_R = 0;               // disable analog functionality on PA
                                        // UART1=priority 2
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFF0FFFFF)|0x00200000; // bits 13-15
...
int UART_InString(char *bufPt, uint16_t max) {
int length=0;
char character;
  character = UART_InChar();
  while(character != CR){
