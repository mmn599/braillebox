...
void (*NextBrailleTask)(void);   // user function
void (*AlarmTask)(void);

// ***************** Timer0A_Init ****************
// Activate Timer0A interrupts to run "solenoid output task" periodically
...
void Timer0A_Disable(void) {
	TIMER0_CTL_R &= ~0x00000001;     // 1) disable timer0A
}
...
// ***************** Timer1A_Init ****************
// Activate Timer1A interrupts to run "solenoid output task" periodically
// Inputs:  task is a pointer to a user function
//          period in 12.5ns units
// Outputs: none
void Timer1A_Init(void(*task)(void), uint32_t period){long sr;
  ...
  NVIC_PRI5_R = (NVIC_PRI4_R&0xFFFF0FFF)|0x00004000; // 8) priority 2
...
void Timer1A_Disable(void) {
	TIMER1_CTL_R &= ~0x00000001;     // 1) disable timer0A
}